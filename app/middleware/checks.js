...
import { 
    access_granted, access_suspended, default_delete_status, ecommerce_header_key, ecommerce_header_token, tag_root, tag_internal_api_key
...
const verifyKey = (req, res, next) => {
    const key = req.headers[ecommerce_header_key] || req.query.key || req.body.key || '';
    if (!key) {
...
const verifyToken = (req, res, next) => {
    let token = req.headers[ecommerce_header_token] || req.query.token || req.body.token || '';
    if (!token) {
